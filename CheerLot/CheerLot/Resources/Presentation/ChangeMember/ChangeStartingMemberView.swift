    ...
    private var selectedMemberNameView: some View {
        VStack(spacing: DynamicLayout.dynamicValuebyHeight(4)) {
            Text("교체 선수")
                ...
                .lineHeightMultipleAdaptPretend(fontType: .semibold, fontSize: 16, lineHeight: 1.3, letterSpacing: -0.04)
            
            Text(changeForPlayer.name)
                .foregroundStyle(Color.black)
                .lineHeightMultipleAdaptPretend(fontType: .bold, fontSize: 24, lineHeight: 1.2, letterSpacing: -0.05)
                .padding(.bottom, DynamicLayout.dynamicValuebyHeight(10))
            
            Text("교체할 선수를 선택해주세요")
                .foregroundStyle(selectedTheme.primaryColor)
                .lineHeightMultipleAdaptPretend(fontType: .medium, fontSize: 14, lineHeight: 1.3, letterSpacing: -0.04)
        }
    }
